...
typedef struct {
	...
	Node** node_array;
	//exit_node : used to check for insructions that other instruction aren't depented on. end of branches.
	//			 help to find the longest paths.
	bool* exit_node;
} Tree;
...
ProgCtx analyzeProg(const unsigned int opsLatency[], InstInfo progTrace[], unsigned int numOfInsts) {
	try {
		...
		//reg_file - array of the instruction that wrote last to the specific reg, intialized to -1 
